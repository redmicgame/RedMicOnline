...
export const GoogleView: React.FC = () => {
    ...
    const baseArtistProfile = allPlayerArtistsAndGroups.find(a => a.id === gameState.activeArtistId);
    ...
    // Check if matching an online artist (not the player)
    let onlineMatchProfile = null;
    let onlineMatchData = null;
    if (!gameState.offlineMode && gameState.onlineArtists) {
        const found = gameState.onlineArtists.find(a => a.name.toLowerCase() === lowerQuery);
        if (found) {
            onlineMatchProfile = {
                id: found.id,
                name: found.name,
                age: found.age || 20,
                fandomName: found.fandomName || 'Fans',
                publicImage: found.publicImage || 50
            };
            onlineMatchData = {
                artistImages: [found.avatar || 'https://ui-avatars.com/api/?background=random'],
                releases: gameState.onlineAlbums?.filter(alb => alb.artistId === found.id) || [],
                songs: gameState.onlineSongs?.filter(s => s.artistId === found.id) || [],
                publicImage: found.publicImage || 50
            };
        }
    }

    const currentProfile = onlineMatchProfile || baseArtistProfile;
    const currentArtistData = onlineMatchData || activeArtistData;

    if (!currentArtistData || !currentProfile) return null;

    const isMatchArtist = lowerQuery === currentProfile.name.toLowerCase();
    const isMatchTour = (lowerQuery.includes('tour') || lowerQuery.includes('tours')) && lowerQuery.includes(currentProfile.name.toLowerCase()) || lowerQuery === 'tour';
    const isMatchNetWorth = lowerQuery.includes('net worth') && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isTmzQuery = lowerQuery.includes('tmz') || lowerQuery.includes('paparazzi');
    const isMatchTmzArtist = isTmzQuery && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchTmzGeneral = isTmzQuery && !isMatchTmzArtist;
    const isMatchBeef = (lowerQuery.includes('beef') || lowerQuery.includes('drama') || lowerQuery.includes('twitter') || lowerQuery.includes('x')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchDating = (lowerQuery.includes('dating') || lowerQuery.includes('boyfriend') || lowerQuery.includes('girlfriend') || lowerQuery.includes('relationships')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchUpcoming = (lowerQuery.includes('album countdown') || lowerQuery.includes('upcoming album') || lowerQuery.includes('new album') || lowerQuery.includes('upcoming release') || lowerQuery.includes('release')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchBillboard = lowerQuery.includes('billboard') && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchOnlyFans = (lowerQuery.includes('onlyfans') || lowerQuery.includes('of')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchCatalog = (lowerQuery.includes('catalog') || lowerQuery.includes('masters')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchAwards = (lowerQuery.includes('grammy') || lowerQuery.includes('award') || lowerQuery.includes('oscars') || lowerQuery.includes('amas') || lowerQuery.includes('vmas')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchCoachella = (lowerQuery.includes('coachella') || lowerQuery.includes('festival') || lowerQuery.includes('gigs')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchSpotifyCharts = (lowerQuery.includes('spotify chart') || lowerQuery.includes('spotify record') || lowerQuery.includes('spotify streams') || lowerQuery.includes('spotify')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    const isMatchPayola = (lowerQuery.includes('payola') || lowerQuery.includes('buying streams') || lowerQuery.includes('scandal')) && lowerQuery.includes(currentProfile.name.toLowerCase());
    ...
    const popularSongs = (currentArtistData.songs || []).filter(s => s.isReleased && !s.remixOfSongId).sort((a, b) => (b.streams || 0) - (a.streams || 0)).slice(0, 3);
    ...
    const estimatedNetWorth = Math.floor(currentArtistData.money + (currentArtistData.hype * 2000) + (currentArtistData.monthlyListeners * 1.5));

    const getPaaAnswer = (index: number) => {
        switch (index) {
            case 0:
                return `${currentProfile.name} rose to fame with their early releases, quickly gaining a massive following in the ${currentProfile.fandomName} fandom. Their unique style and consistent output have kept them relevant.`;
            case 1:
                ...
                return `${currentProfile.name} has several highly successful tracks, but definitive data on their absolute biggest hit is currently pending new chart updates.`;
            case 2:
                const hasTour = currentArtistData.tours && currentArtistData.tours.some(t => t.status === 'planning' || t.status === 'active');
                if (hasTour) {
                    const tourName = currentArtistData.tours.find(t => t.status === 'active' || t.status === 'planning')?.name;
                    return `Yes, ${currentProfile.name} is currently focused on their "${tourName}" tour, selling out arenas and stadiums around the world.`;
                }
                return `There are currently no official tour announcements for ${currentProfile.name}. Fans are eagerly awaiting news on future live dates.`;
            case 3:
                return `Details about ${currentProfile.name}'s extended family and personal relationships are kept mostly private to respect their boundaries.`;
            default:
                return '';
        }
    };
                ...
                <div className="bg-white text-black min-h-screen font-sans relative pb-16">
                    ...
                    <main className="max-w-4xl mx-auto p-4 flex flex-col md:flex-row-reverse gap-8 mt-4">
                        <div className="w-full md:w-[22em] flex-shrink-0 border border-zinc-300 bg-[#f8f9fa] p-2 text-sm self-start">
                            <h2 className="text-center font-bold text-lg mb-2">{currentProfile.name}</h2>
                            {currentArtistData.artistImages && currentArtistData.artistImages.length > 0 ? (
                                <img src={currentArtistData.artistImages[0]} alt={currentProfile.name} className="w-full aspect-square object-cover border border-zinc-300" />
                            ) : (
                            ...
                            <table className="w-full my-2 infobox-table">
                                <tbody>
                                    <tr className="border-b border-zinc-200">
                                        ...
                                        <td className="py-1 text-black">{currentProfile.age} years old</td>
                                    </tr>
                                    <tr className="border-b border-zinc-200">
                    ...
                    <div className="max-w-2xl mx-auto p-4 sm:p-6 mt-4">
                        ...
                        {currentArtistData.artistImages && currentArtistData.artistImages.length > 0 && (
                            <div className="w-full h-64 md:h-96 relative border-4 border-red-600 rotate-[-1deg] mb-6">
                                <img src={currentArtistData.artistImages[Math.floor(Math.random() * currentArtistData.artistImages.length)]} className="w-full h-full object-cover grayscale opacity-90 contrast-125" alt="Paparazzi shot" />
            ...
            return (
                <div className="bg-white text-black min-h-screen font-sans relative pb-16">
                    ...
                    <main className="max-w-3xl mx-auto px-4 py-8">
                        ...
                        {currentArtistData.artistImages && currentArtistData.artistImages.length > 0 && (
                            <img src={currentArtistData.artistImages[0]} className="w-full aspect-[16/9] object-cover mb-8 shadow-sm" alt="Artist performance" />
                        )}
                                ...
                                <div className="flex-1 pb-2">
                                    ...
                                    {currentArtistData.artistImages && currentArtistData.artistImages.length > 0 && (
                                        <div className="mt-3 rounded-2xl overflow-hidden border border-gray-200">
                                            <img src={currentArtistData.artistImages[0]} className="w-full aspect-[4/3] object-cover" alt="Post media" />
                                        </div>
                                    )}
                ...
                <main className="max-w-2xl mx-auto w-full xl:max-w-4xl p-4 sm:p-6 mt-2">
                    ...
                    {currentArtistData.artistImages && currentArtistData.artistImages.length > 0 && (
                        <figure className="mb-8 w-full">
                            <img src={currentArtistData.artistImages[0]} className="w-full h-auto aspect-video object-cover rounded-xl shadow-sm" alt="Article representative" />
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    ...
                    {activeTab === 'Images' && (
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                            {(currentArtistData.artistImages || []).map((img, i) => (
                            ...
                            {Array.from({ length: Math.max(0, 12 - (currentArtistData.artistImages || []).length) }).map((_, i) => (
                    ...
                    {activeTab === 'All' && isMatchArtist && (currentArtistData.publicImage ?? 80) <= 40 && (
                    ...
                    {activeTab === 'All' && isMatchArtist && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                            <div className="flex space-x-1 p-2 overflow-x-auto bg-gray-50">
                                {(currentArtistData.artistImages || []).slice(0, 4).map((img, i) => (
                            ...
                            <div className="p-4 bg-white">
                                <h1 className="text-2xl font-normal text-gray-900 mb-1">{currentProfile.name}</h1>
                                <p className="text-sm text-gray-500 mb-4">{currentProfile.fandomName} fandom</p>
                                
                                <p className="text-sm text-gray-700 leading-relaxed line-clamp-3 mb-2">
                                    {currentProfile.name} is a global recording artist known for breaking chart records and defining the current era of popular music.
                                </p>
                                <a href="#" onClick={(e) => { e.preventDefault(); setReadingArticle({
                                    title: currentProfile.name,
                                    url: `https://en.wikipedia.org/wiki/${currentProfile.name.replace(/ /g, '_')}`,
                                    content: `${currentProfile.name} (born Year Unknown) is a widely recognized global recording artist. Known for a distinctive style and massive achievements on global charts.\n\nEarly Life and Career\nThe origins and early development of their career show a clear trajectory toward mainstream success. During the initial phases, ${currentProfile.name} established a core fanbase known as ${currentProfile.fandomName}.\n\nLegacy\nWith numerous albums under their belt, the cultural impact of ${currentProfile.name} is still being measured today, with numerous accolades and critical recognition across the music industry.`,
                                    type: 'wikipedia',
                                    extraData: { releasesLength: currentArtistData.releases?.length || 0 }
                                ...
                                <div className="mt-4 border-t border-gray-100 pt-4 space-y-2">
                                    <div className="flex text-sm">
                                        ...
                                        <span className="text-gray-600">{currentProfile.age} years old</span>
                                    </div>
                                    <div className="flex text-sm">
                                        <span className="font-bold w-24 text-gray-900">Albums:</span>
                                        <span className="text-blue-600 hover:underline">{(currentArtistData.releases || []).filter(r => r.type === 'Album').length}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {isMatchTmzArtist ? (
                                <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                                    ...
                                    <p className="text-gray-800 text-lg italic mb-2">"{currentProfile.name} Caught on Camera!"</p>
                                    <p className="text-sm text-gray-700 font-medium max-w-prose">
                                        See the latest photos and rumors surrounding the global superstar. Inside sources say {currentProfile.name} was spotted leaving a private party early this morning.
                                    </p>
                                    <button className="mt-4 px-4 py-2 bg-red-600 text-white font-bold text-sm uppercase hover:bg-black transition-colors" onClick={() => setReadingArticle({
                                        title: `${currentProfile.name} CAUGHT IN LA!`,
                                        url: 'https://www.tmz.com/exclusive',
                                        content: `We got ${currentProfile.name} out in Los Angeles and you won't believe what happened next.\n\nThe global superstar was seen leaving a highly exclusive afterparty surrounded by security. Fans were hounding the exit but our sources got the exclusive shots.\n\nIs ${currentProfile.name} preparing for a massive new release or just blowing off steam? We're hearing whispers of major label drama behind the scenes...`,
                                        type: 'tmz'
                    ...
                    {activeTab === 'All' && isMatchBeef && (
                        <div className="border-t-4 border-black border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4 space-y-4">
                            ...
                            {currentArtistData.fanWarStatus ? (
                                <div className="flex flex-col gap-2">
                                    <h3 className="text-lg font-medium text-blue-600 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                        title: `${currentProfile.name} vs ${currentArtistData.fanWarStatus?.targetArtistName} Fan War Reaches Boiling Point!`,
                                        url: `https://www.popcrave.com/${currentProfile.name}-vs-${currentArtistData.fanWarStatus?.targetArtistName}`,
                                        content: `The timeline is in shambles as the stan bases of ${currentProfile.name} and ${currentArtistData.fanWarStatus?.targetArtistName} go head to head.\n\nIt all started after some shady subliminals and now the fanbases are fully mobilizing. The beef is officially ON and the memes are brutal.`,
                                        type: 'popcrave'
                                    })}>
                                        {currentProfile.name} vs {currentArtistData.fanWarStatus.targetArtistName} Fan War Reaches Boiling Point!
                                    </h3>
                                    <p className="text-sm text-gray-700 font-medium bg-gray-50 p-3 border-l-4 border-blue-500 rounded-r-lg">
                                        Active feud confirmed: Your fans are currently warring with {currentArtistData.fanWarStatus.targetArtistName}'s fan base. This will continue for {currentArtistData.fanWarStatus.weeksRemaining} more weeks.
                                    </p>
                                    <div className="text-xs text-gray-500 uppercase font-bold tracking-widest mt-2">{currentArtistData.fanWarStatus.targetArtistName} | Twitter Drama</div>
                                </div>
                            ) : (
                                <div className="flex flex-col gap-2">
                                    <p className="text-sm text-gray-600 font-medium">Surprisingly, there are no active major feuds or twitter battles happening right now for {currentProfile.name}.</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'All' && isMatchTour && (
                        <div className="space-y-4">
                            ...
                            {currentArtistData.tours && currentArtistData.tours.length > 0 ? (
                                <div className="space-y-3">
                                    {currentArtistData.tours.map((t, i) => (
                    ...
                    {activeTab === 'All' && isMatchDating && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                            <h2 className="text-sm font-medium text-gray-600 uppercase tracking-wider mb-2">{currentProfile.name} Dating History</h2>
                            <h3 className="text-xl font-bold text-blue-600 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                title: `Who is ${currentProfile.name} dating right now?`,
                                url: 'https://www.eonline.com/news',
                                content: `Fans have been tracking ${currentProfile.name}'s relationship status closely.\n\nRecent rumors suggest that the global superstar might be seeing someone new, although reps have declined to comment officially.\n\nFrom high-profile romances to quiet flings, this artist always keeps us guessing.`,
                                type: 'news'
                            })}>
                                Everything to know about {currentProfile.name}'s relationship status
                            </h3>
                            <p className="text-sm text-gray-700 mt-2">
                                Fans are constantly speculating about who the global star is currently seeing. From high-profile relationships to quiet flings, {currentProfile.name}'s dating life is highly publicized.
                            </p>
                            <button className="mt-3 text-sm font-medium text-blue-600 hover:text-blue-800" onClick={() => setReadingArticle({
                                title: `A Complete Timeline of ${currentProfile.name}'s Dating History`,
                                url: 'https://www.usmagazine.com/celebrity-news',
                                content: `Let's take a look back at ${currentProfile.name}'s highly publicized romantic history.\n\nFrom their explosive first major public relationship to the quiet dating rumors of today, they've always captured headlines.`,
                                type: 'news'
                    ...
                    {activeTab === 'All' && isMatchUpcoming && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4 space-y-2">
                            ...
                            {currentArtistData.releases && currentArtistData.releases.filter(r => !r.isReleased).length > 0 ? (
                                <div className="space-y-3">
                                    {currentArtistData.releases.filter(r => !r.isReleased).slice(0, 3).map((r, i) => (
                                        <div key={i} className="flex gap-4 items-center">
                                            ...
                                            <div>
                                                <div className="font-semibold text-blue-600 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                                    title: `${currentProfile.name} Teases Upcoming Release "${r.title}"`,
                                ...
                                <p className="text-sm text-gray-600">There are currently no official upcoming releases announced for {currentProfile.name}.</p>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'All' && isMatchBillboard && (
                        <div className="border-t-4 border-blue-600 border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 hover:text-blue-600 cursor-pointer" onClick={() => setReadingArticle({
                                title: `${currentProfile.name} Achieves Major Milestone on Billboard Charts`,
                                url: 'https://www.billboard.com/charts',
                                content: `Chart history has been made again.\n\n${currentProfile.name} continues their streak of incredible charting hits. With this latest entry, they cement their status as one of the defining artists of the decade.`,
                                type: 'news'
                            })}>
                                {currentProfile.name} Chart History
                            </h3>
                            <p className="text-sm text-gray-600 mt-2">
                                Check out {currentProfile.name}'s peak positions, total weeks on chart, and all historic entries on the Hot 100.
                            </p>
                            <button className="mt-3 text-sm font-bold text-blue-600" onClick={() => setReadingArticle({
                                title: `The Biggest Hits of ${currentProfile.name}'s Career`,
                                url: 'https://www.billboard.com/lists',
                                content: `A ranking of their absolute biggest smashes on the Billboard Hot 100.\n\nFrom their debut entries to their multi-week number one hits, ${currentProfile.name} has proven they know how to stay on top.`,
                                type: 'news'
                    ...
                    {activeTab === 'All' && isMatchOnlyFans && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4 border-t-4 border-[#00AFF0]">
                            ...
                            {currentArtistData.onlyfans ? (
                                <div>
                                    <h3 className="font-semibold text-lg hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                        title: `${currentProfile.name} is Officially on OnlyFans!`,
                                        url: 'https://www.complex.com/music',
                                        content: `The news broke the internet.\n\n${currentProfile.name} has officially launched an OnlyFans account. Fans flocking to the platform can expect exclusive, behind-the-scenes content that they've never seen before.`,
                                        type: 'news'
                                    })}>
                                        Subscribe to {currentProfile.name}'s Official OnlyFans
                                    </h3>
                                    ...
                                    <button className="mt-3 px-4 py-2 bg-[#00AFF0] text-white font-bold rounded hover:bg-blue-600 transition-colors" onClick={() => setReadingArticle({
                                        title: `${currentProfile.name}'s Subscriber Count Explodes`,
                                        url: 'https://www.tmz.com/onlyfans',
                                        content: `Sources tell us ${currentProfile.name} is making massive bank on the platform.\n\nTheir recent content drop caused the site to briefly crash for several users.`,
                                        type: 'tmz'
                                    })}>View Profile</button>
                                </div>
                            ) : (
                                <div>
                                    <h3 className="font-semibold text-lg text-gray-800">Did {currentProfile.name} join OnlyFans?</h3>
                                    <p className="text-sm text-gray-600 mt-1">There is currently no official OnlyFans account for {currentProfile.name}. Many fans have requested it, but it remains unconfirmed.</p>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
                            ...
                            <div className="p-4">
                                <h3 className="font-bold text-xl text-blue-600 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                    title: `Inside the value of ${currentProfile.name}'s music catalog`,
                                    url: 'https://www.musicbusinessworldwide.com/news',
                                    content: `Music catalogs are booming assets right now.\n\nIndustry analysts state that ${currentProfile.name}'s master recordings could be valued in the hundreds of millions. Will they sell to an investment firm or retain independent ownership?\n\nThis decision could reshape their financial future forever.`,
                                    type: 'news'
                                })}>
                                    Inside the value of {currentProfile.name}'s music catalog
                                </h3>
                                <p className="text-sm text-gray-600 mt-2">
                                    With master recordings valued in the millions, industry analysts are speculating whether {currentProfile.name} will sell their entire catalog to an investment firm or retain full ownership.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'All' && isMatchAwards && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm p-4 bg-gradient-to-br from-yellow-50 to-white border-t-4 border-yellow-500">
                            ...
                            <p className="text-sm text-gray-700 mb-3">
                                {currentProfile.name} has been highly decorated throughout their career holding several prestigious awards.
                            </p>
                            <div className="space-y-2 mb-3">
                                <div className="flex justify-between items-center text-sm border-b pb-1">
                                    <span className="font-medium">GRAMMY Wins</span>
                                    <span className="font-bold">{currentArtistData.grammyHistory ? currentArtistData.grammyHistory.length : 0}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm border-b pb-1">
                                    <span className="font-medium">AMA Wins</span>
                                    <span className="font-bold">{currentArtistData.amaHistory ? currentArtistData.amaHistory.length : 0}</span>
                                </div>
                            </div>
                            <button className="text-sm font-bold text-yellow-700 hover:text-yellow-900 w-full text-center" onClick={() => setReadingArticle({
                                title: `${currentProfile.name}'s Historic Award Run`,
                                url: 'https://www.grammy.com/news',
                                content: `An unforgettable night for music.\n\n${currentProfile.name} swept major categories, proving their dominance in the industry space. Their heartfelt acceptance speech has already gone viral.\n\nFans are celebrating this massive milestone.`,
                                type: 'news'
                            })}>
                                View GRAMMYs
                            </button>
                    ...
                    {activeTab === 'All' && isMatchCoachella && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                            ...
                            <h3 className="font-medium text-blue-600 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                title: `${currentProfile.name} Rumored to Headline Major Festival`,
                                url: 'https://www.rollingstone.com/music-festivals',
                                content: `The rumors are getting stronger.\n\nMultiple sources report that ${currentProfile.name} is in final negotiations to headline one of the year's biggest music festivals.\n\nIf true, it will be their biggest live performance to date.`,
                                type: 'rollingstone'
                            })}>
                                Will {currentProfile.name} perform at Coachella this year?
                            </h3>
                            <p className="text-sm text-gray-600 mt-2 line-clamp-3">
                                Festival lineups are highly anticipated, and {currentProfile.name} is consistently one of the most requested headliners in the world. Check out upcoming gigs and festival bookings.
                            </p>
                        </div>
                    )}
                        ...
                        <div className="border-t-4 border-[#1DB954] border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-3">
                                <div className="bg-gray-50 p-3 rounded">
                                    ...
                                    <div className="text-xl font-black">{formatNumber(currentArtistData.monthlyListeners)}</div>
                                </div>
                                <div className="bg-gray-50 p-3 rounded">
                                    <div className="text-xs text-gray-500 font-bold uppercase">Followers</div>
                                    <div className="text-xl font-black">{formatNumber(currentArtistData.followers || 0)}</div>
                                </div>
                            </div>
                            <button className="w-full py-2 bg-[#1DB954] text-white font-bold rounded-full hover:bg-green-600 transition-colors" onClick={() => setReadingArticle({
                                title: `${currentProfile.name} Breaks Spotify Streaming Record`,
                                url: 'https://newsroom.spotify.com',
                                content: `Numbers don't lie.\n\n${currentProfile.name} has just surpassed massive milestones on the platform. Earning millions of monthly listeners, they are officially one of the top trending artists globally this week.`,
                                type: 'news'
                            })}>
                    ...
                    {activeTab === 'All' && isMatchPayola && (
                        <div className="border border-red-300 bg-red-50 rounded-xl overflow-hidden shadow-sm p-4">
                            ...
                            <h3 className="font-medium text-red-900 hover:underline cursor-pointer" onClick={() => setReadingArticle({
                                title: `Investigating ${currentProfile.name}'s Streaming Numbers`,
                                url: `https://www.musicindustrywatch.com/scandal/${currentProfile.name.replace(/ /g, '-')}`,
                                content: `Are the numbers real? Several investigative journalists have started looking into widespread allegations of payola and "botting" streams across the industry.\n\nWhile representatives for ${currentProfile.name} have fiercely denied any wrongdoing, the rumors persist on social media timelines.`,
                                type: 'news'
                            })}>
                                Did {currentProfile.name} use payola for their latest hit?
                            </h3>
                            <p className="text-sm text-red-800 mt-2">
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4 mb-4">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    const canAfford = currentArtistData.popularity >= deal.reqPop && (currentArtistData.publicImage ?? 80) >= deal.reqImg;
                                    const isSigned = currentArtistData.signedBrandDeals?.includes(deal.id);
                                    return (
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4 mb-4">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    const canAfford = currentArtistData.popularity >= game.reqPop;
                                    const isSigned = currentArtistData.signedVideoGames?.includes(game.id);
                                    return (
                    ...
                    {activeTab === 'All' && isMatchNetWorth && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white p-4">
                            <h2 className="text-sm font-medium text-gray-600 uppercase tracking-wider mb-2">{currentProfile.name} Net Worth</h2>
                    ...
                    {activeTab === 'All' && (isMatchArtist || isMatchTour || activeQuery) && (
                        <div className="border border-gray-200 rounded-xl overflow-hidden mt-6 bg-white shadow-sm">
                            ...
                            <div className="divide-y divide-gray-100">
                                {[
                                    `How did ${currentProfile.name} get famous?`,
                                    `What is ${currentProfile.name}'s biggest hit?`,
                                    `Is ${currentProfile.name} going on tour this year?`,
                                    `Who is ${currentProfile.name} related to?`
                                ].map((question, i) => (
                    ...
                    {activeTab === 'All' && (isMatchArtist || isMatchTour || isMatchAnySpecial) && (
                        <div className="mt-8 pt-6 border-t border-gray-200">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[`${currentProfile.name} net worth`, `${currentProfile.name} dating`, `${currentProfile.name} newest album`, `${currentProfile.name} tour dates`, `${currentProfile.name} TMZ controversy`, `${currentProfile.name} twitter drama`, `${currentProfile.name} Spotify record`, `${currentProfile.name} masters catalog`].map((str, i) => (
